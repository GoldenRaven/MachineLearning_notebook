...
| sklearn类名      | 损失函数(loss)              | 正则项(penalty)                        | 算法名称           | 实现方法                                                 |
|------------------+-----------------------------+----------------------------------------+--------------------+----------------------------------------------------------|
| LinearRegression | squared_loss                | 无                                     | 普通最小二乘法     | *X* 矩阵的SVD分解（闭式解）                              |
| Ridge            | squared_loss                | l_{2}正则项                            | 岭回归             | ？                                                       |
| Lasso            | squared_loss                | l_{1}正则项                            | 套索回归           | 坐标下降（迭代优化）                                     |
| ElasticNet       | squared_loss                | \rho l_{1}+(1-\rho)l_{2}               | 弹性网络           | ？                                                       |
| SGDRegressor     | squared_loss                | {None, 'l_{2}', 'l_{1}', 'elasticnet'} | 以上四种算法       | 随机梯度下降                                             |
|                  | huber                       | {None, 'l_{2}', 'l_{1}', 'elasticnet'} | Huber回归          | 随机梯度下降                                             |
|                  | epsilon_insensitive         | {None, 'l_{2}', 'l_{1}', 'elasticnet'} | 线性支持向量机回归 | 随机梯度下降                                             |
|                  | squared_epsilon_insensitive | {None, 'l_{2}', 'l_{1}', 'elasticnet'} | 无                 | 随机梯度下降                                             |
| LinearSVR        | epsilon_insensitive         | l_{1}正则项                            | 线性支持向量机回归 | 基于liblinear的优化算法，快速收敛                        |
|                  | squared_epsilon_insensitive | l_{2}正则项                            | 线性支持向量机回归 | 基于liblinear的优化算法，快速收敛                        |
| SVR              | epsilon_insensitive         | l_{2}正则项                            | 支持向量机回归     | kernel={linear, rbf, poly ,sigmoid ,precomputed ,自定义} |
