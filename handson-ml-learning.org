...
** 第五章 支持向量机（SVM, suppotive vector machine）
#+BEGIN_QUOTE
只读一本书不是学习，而是娱乐。
#+END_QUOTE
支持向量机可以用于 ~线性~ 、 ~非线性~ 的 ~分类~ 和 ~回归~ 任务，也可以用
于异常值检测任务。SVM是机器学习领域最受欢迎的模型之一，特别适用于中小型复杂
数据集的分类。
...
持），这些实例称为支持向量。预测结果时只涉及扶持向量，而不涉及整个训练集。
...
+--------------------------------+--------------------------------------------+----------------------------+------------+------------+----------+--------------+---------------------------------------+------------------------------+----------------------------------------------------+
|                                | 调用方法                                   | 时间复杂度                 | 需要缩放？ | 支持核外？ | 运算速度 | 支持核技巧？ | 优点                                  | 缺点                         | 注意                                               |
+--------------------------------+--------------------------------------------+----------------------------+------------+------------+----------+--------------+---------------------------------------+------------------------------+----------------------------------------------------+
| LinearSVC类                    | LinearSVC(C=1, loss='hinge')               | O(m*n)                     | 是         | 否         | 快       | 否           | 基于liblinear库实现的优化算法，收敛快 | 不支持核技巧                 | 要先减去平均值使训练集集中，还要设置超参loss和dual |
+--------------------------------+--------------------------------------------+----------------------------+------------+------------+----------+--------------+---------------------------------------+------------------------------+----------------------------------------------------+
| SVC类的linear kernel           | SVC(kernel='linear', C=1)                  | O(m^{2}*n)与O(m^{3}*n)之间 | 是         | 否         | 慢       | 是           | 基于libsvm库，支持核技巧              | 只适用于复杂但中小型的训练集 | 不要用在大型数据集上（超过十万）                   |
+--------------------------------+--------------------------------------------+----------------------------+------------+------------+----------+--------------+---------------------------------------+------------------------------+----------------------------------------------------+
| SGDClassifier类的hinge损失函数 | SGDClassifier(loss='hinge', alpha=1/(m*C)) | O(m*n)                     | 是         | 是         | 慢       | 否           | 对大型数据集有效，对在线分类任务有效  | 不支持核技巧                 | 成本函数要可导才能用梯度下降                       |
+--------------------------------+--------------------------------------------+----------------------------+------------+------------+----------+--------------+---------------------------------------+------------------------------+----------------------------------------------------+
...
注意：
  ...
  2. 与Logistic回归分类器不同，SVM不会输出类别概率，可以设probability=True，
     来用Logistic回归来对SVM校准，可以获得predict_proba()和predict_log_proba()方法
#+end_example
