...
注意：
- 高次幂项也包括特征的交叉项
- 作用PolynomialFeatures类要小心特征数量爆炸！
#+END_EXAMPLE
**** 4.2.2 学习曲线
...
也叫吉洪诺夫正则化，在成本函数中添加一个正则项 \alpha \sum_{i=1}^n \theta_i^2。
...
#+END_CENTER
超参数\alpha 控制正则化程度，\alpha=0时回复到线性模型，\alpha 非常大时所有权重都接近于零，结果是一条穿过数据平均值的水平线。正则项是权重向量\textbf{\theta}的l_{2}范数平方的一半。
#+BEGIN_EXAMPLE
注意：
1. 求和从i=1开始，对偏置项不正则化。
