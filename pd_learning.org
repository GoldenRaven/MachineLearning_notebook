...
df['FareBins'] = pd.cut(df['Fare'], 4) # discretize continious to bins
df['AgeBins'] = pd.qcut(df['Age'].astype(int), 5) # discretize continious to bins
df['Title'] = df['Title'].apply(lambda x: 'Misc' if title_name[x] else x) # clean titles with small amount
#+END_SRC
*** Converting
